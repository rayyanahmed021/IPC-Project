	...
	do
	{
		...
		case 3:
		{
			...
			else if (matchIndex != -1)
			{
				...
				{
					updateTicketStatus(accountTicket->tickets, accountTicket->accounts[matchIndex].accountNumber, accountTicket->TICKET_MAX_SIZE, 0, name, 'A');
		...
		{
			putchar('\n');
			matchIndex = findTicketIndexByTickNum(0, "ticket number", accountTicket->tickets, accountTicket->TICKET_MAX_SIZE, 1);

			if (matchIndex != -1)
			{
				menuUpdateTcktAgent(&accountTicket->tickets[matchIndex], name, accountTicket->TICKET_MAX_SIZE);
			}
			else
			{
